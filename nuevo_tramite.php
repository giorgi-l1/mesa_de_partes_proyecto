...
$query_tipos = "SELECT id_tipo_tramite, nombre_tramite, descripcion FROM tipos_tramite ORDER BY nombre_tramite ASC";
...
<body>
    ...
    <div class="container-sm">
        <div class="panel">
            ...
            if (isset($_GET['error'])) {
                $error = $_GET['error'];
                if ($error == 'formato') echo '<div class="alert" style="background:#f8d7da; color:#721c24;">El archivo adjunto debe ser obligatoriamente en formato PDF.</div>';
                if ($error == 'peso') echo '<div class="alert" style="background:#f8d7da; color:#721c24;">El archivo excede el límite máximo de 5MB. Utilice el enlace externo.</div>';
                if ($error == 'db') echo '<div class="alert" style="background:#f8d7da; color:#721c24;">Error al registrar el trámite. Intente nuevamente.</div>';
            }
            ...
            <form action="p_nuevo_tramite.php" method="POST" enctype="multipart/form-data">
                <div class="form-grid">
                    ...
                    <div class="form-group full-width">
                        ...
                        <input type="text" name="asunto" required placeholder="Ej: Solicitud de Constancia de Matrícula" maxlength="250">
                    </div>

                    <div class="form-group full-width">
                        <label>Fundamentación del Pedido *</label>
                        <textarea name="descripcion_motivo" rows="4" required placeholder="Explique detalladamente el motivo de su solicitud..."></textarea>
                    </div>

                    <div class="form-group full-width" style="margin-top: 15px; border-top: 1px solid #eee; padding-top: 15px;">
                        <h3 style="color: var(--dorado-arena); margin-bottom: 10px;">Sustento Adjunto</h3>
                    </div>
