...
if (!$res_staff || mysqli_num_rows($res_staff) == 0) {
    // El usuario tiene credenciales de personal, pero no está asignado a ninguna oficina
...
//    (Se excluyen los ya Atendidos/Finalizados: id_estado = 5)
// ----------------------------------------------------
...
// ----------------------------------------------------
// 5. LISTADO DE TRÁMITES DE LA BANDEJA (columnas solicitadas:
//    IDTRAMITE, NOMBRE, TIPODEUSUARIO, ASUNTO, DESCRIPCION, AREA)
// ----------------------------------------------------
...
// ----------------------------------------------------
// 6. LISTA DE OFICINAS DISPONIBLES PARA DERIVAR (todas menos la propia)
// ----------------------------------------------------
    ...
    <div class="container">
        ...
        <div class="panel">
            <div style="display:flex; justify-content: space-between; align-items:center; flex-wrap: wrap; gap: 10px;">
                <h2 class="panel-title" style="border:none; margin-bottom:0; padding-bottom:0;">
            ...
            <?php if ($mensaje): ?>
                <div class="mensaje-flash mensaje-<?php echo $tipo_mensaje; ?>"><?php echo htmlspecialchars($mensaje); ?></div>
            <?php endif; ?>
                        ...
                        <?php if ($res_bandeja && mysqli_num_rows($res_bandeja) > 0): ?>
                            <?php while ($t = mysqli_fetch_assoc($res_bandeja)): ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="mesa_exportar_pdf.php?id=<?php echo $t['id_tramite']; ?>" target="_blank" class="btn-icono-pdf" title="Ver constancia en PDF">📄</a>
                                    </td>
                                    <?php if ($puede_finalizar || $puede_derivar): ?>
                                        <td class="col-acciones">
                                            <?php if ($puede_finalizar): ?>
                                                <form action="mesa_finalizar_tramite.php" method="POST" onsubmit="return confirm('¿Confirmas que el trámite <?php echo htmlspecialchars($t['numero_expediente']); ?> ha concluido su gestión?');" style="display:inline;">
                ...
                <div class="paginacion">
                    ...
                    <div class="paginacion-controles">
                        <?php if ($pagina_actual > 1): ?>
                            <a href="principal_mesa.php?pagina=<?php echo $pagina_actual - 1; ?>" class="pagina-btn">&larr; Anterior</a>
                        <?php else: ?>
                            <span class="pagina-btn pagina-disabled">&larr; Anterior</span>
                        <?php endif; ?>

                        <?php for ($p = 1; $p <= $total_paginas; $p++): ?>
                            <a href="principal_mesa.php?pagina=<?php echo $p; ?>" class="pagina-btn <?php echo ($p == $pagina_actual) ? 'pagina-activa' : ''; ?>"><?php echo $p; ?></a>
                        <?php endfor; ?>

                        <?php if ($pagina_actual < $total_paginas): ?>
                            <a href="principal_mesa.php?pagina=<?php echo $pagina_actual + 1; ?>" class="pagina-btn">Siguiente &rarr;</a>
                        <?php else: ?>
        ...
        <div class="modal-box">
            ...
            <form action="mesa_derivar_tramite.php" method="POST">
                ...
                <div class="form-group-modal">
                    ...
                    <select name="id_oficina_destino" id="id_oficina_destino" required>
                        ...
                        <?php foreach ($oficinas_disponibles as $of): ?>
                            <option value="<?php echo $of['id_oficina']; ?>"><?php echo htmlspecialchars($of['nombre_oficina']); ?> (<?php echo htmlspecialchars($of['siglas']); ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group-modal">
                    ...
                    <textarea name="observaciones" id="observaciones" rows="3" placeholder="Motivo de la derivación..."></textarea>
                </div>
